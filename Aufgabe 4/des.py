...
def main():
    ...
    parser.add_argument(
        ...
        dest='key',
        type=str,
        default=hex(rand.randint(0, 2**64-1)),
        help='Schlüssel, sonst random Schlüssel'
    )
    parser.add_argument(
        '--e',
        dest='enrcypt',
        action='store_true',
        default='false',
        help='Verschlüsselt den angegebenen Text'
    )
    parser.add_argument(
        '--d',
        dest='decrypt',
        action='store_true',
        default='false',
        help='Entschlüsselt den angegebenen Text'
    )

    args = parser.parse_args()

    # Input Text einlesen und danach schließen
    file = open(args.input, "rb")
    input_blocks = getblocks(file.read(), 128)
    file.close

    round_keys = makeroundkeys(key=int(args.key, 16))
